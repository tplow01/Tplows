...
export default function GalleryItemPage({ data, backHref, backLabel }: Props) {
  if (!data) {
    return (
      <div style={{ paddingTop: '72px', backgroundColor: 'var(--surface-page)', minHeight: '100vh', padding: '120px 40px' }}>
        <p style={{ fontFamily: 'var(--font-dm-sans), sans-serif', color: 'var(--text-primary-subtle)' }}>Content coming soon.</p>
      </div>
    )
  }

  return (
    <div style={{ paddingTop: '72px', backgroundColor: 'var(--surface-page)', minHeight: '100vh' }}>
      {/* Hero */}
      <section
        style={{
      ...
      >
        <TransitionLink
          href={backHref}
          style={{ fontFamily: 'var(--font-dm-sans), sans-serif', fontSize: '13px', color: 'var(--text-inverse-soft)', textDecoration: 'none', display: 'inline-block', marginBottom: '32px' }}
        >
          ← {backLabel}
        </TransitionLink>
        {data.category && (
          <div
            ...
            style={{ fontSize: '12px', letterSpacing: '0.16em', textTransform: 'uppercase', color: 'var(--c-orange)', marginBottom: '16px' }}
          >
            {data.category}
          </div>
        )}
      ...
      {data.description && (
        <section style={{ padding: 'clamp(48px, 6vw, 80px) 40px', maxWidth: '700px' }}>
          <div
            style={{ fontFamily: 'var(--font-dm-sans), sans-serif', fontSize: '17px', lineHeight: 1.75, color: 'var(--text-primary-muted)' }}
          >
